...
5. [Install direnv](https://direnv.net/). I used [homebrew](https://brew.sh/). Optional, but recommended, as it allows you to automatically set some environment variables that make working with Ansible easier. Next time you `cd` into the main directory, direnv will prompt you to load the `.envrc` file. Type `direnv allow`. This should create a local Python virtual environment and tell Ansible Vault to look for a) the encryption key in the `.vault` file and b) the inventory of hosts in `inventory.ini`. Then you never have to pass in `-i` or `--ask-vault-pass` cli params when running `ansible-playbook`.
6. Install Ansible. Running `make` should install the right version (into the virtual env, if you used direnv). Or you can just follow [the official instructions](https://docs.ansible.com/ansible/latest/installation_guide/installation_distros.html). You may need to re-run `make` if your system Python version is updated, as that can break the virtual env.
7. If, like me, you use [Cloudflare](https://www.cloudflare.com/) for their free DNS service, [create an API key](https://github.com/caddy-dns/cloudflare#authenticating), so Caddy can configure a TLS cert without having to listen on port 80.
